...
async function restartProcess(sock, chatId, message) {
    try {
        await sock.sendMessage(chatId, { text: '☑️ Update complete! Restarting…' }, { quoted: message });
    } catch {}
    try {
        // Preferred: PM2
...
async function updateCommand(sock, chatId, message, zipOverride) {
    ...
    try {
        ...
        if (await hasGitRepo()) {
            ...
            const summary = alreadyUpToDate ? `☑️ Already up to date: ${newRev}` : `☑️ Updated to ${newRev}`;
        ...
        try {
            ...
            await sock.sendMessage(chatId, { text: `☑️ Update done. Restarting…` }, { quoted: message });
        } catch {
            await sock.sendMessage(chatId, { text: '☑️ Restared Successfully\n Type .ping to check latest version.' }, { quoted: message });
        }
        await restartProcess(sock, chatId, message);
    } catch (err) {
        console.error('Update failed:', err);
        await sock.sendMessage(chatId, { text: `⚙️ Update failed:\n${String(err.message || err)}` }, { quoted: message });
    }
}
